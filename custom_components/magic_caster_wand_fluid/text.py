...
async def async_setup_entry(
    ...
    async_add_entities([McwAliasTextEntity(address, data)])
...
class McwAliasTextEntity(RestoreText):
    ...
    def __init__(self, address: str, data: dict) -> None:
        """Initialize the text entity."""
        self._address = address
        self._data = data
        self._identifier = address.replace(":", "")[-8:]
        self._attr_name = "Alias"
        self._attr_unique_id = f"mcwf_{self._identifier}_alias"
        self._attr_native_max = 32
        self._attr_native_min = 0
        self._attr_mode = "text"
        self._attr_native_value = data.get("wand_alias", self._identifier)
        self._data["wand_alias_entity"] = self
    ...
    async def async_set_value(self, value: str) -> None:
        """Set the text value."""
        alias = value.strip() or self._identifier
        self._attr_native_value = alias
        self._data["wand_alias"] = alias
    ...
    async def async_added_to_hass(self) -> None:
        ...
        self._attr_native_value = (last_text_data.native_value or "").strip() or self._identifier
        self._data["wand_alias"] = self._attr_native_value
        stream = self._data.get("fluid_stream")
        if stream is not None:
            stream.publish_config_update()
