...
class McwOptionsFlowHandler(OptionsFlow):
    ...
    async def async_step_init(
        ...
        for option_key, option in FLUID_CONFIG_OPTIONS.items():
            ...
            default = self._config_entry.options.get(option_key, option["default"])
            if option_type is bool:
                validator = bool
            else:
                validator = vol.All(
                    vol.Coerce(option_type),
                    vol.Range(min=option.get("min"), max=option.get("max")),
                )
            options_schema[vol.Required(option_key, default=default)] = validator
